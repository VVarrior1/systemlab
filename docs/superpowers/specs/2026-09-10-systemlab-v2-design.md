...
## 13. v2.1 addendum (2026-09-11): closing the 6.5/10 gaps

The v2 re-judge scored 6.5/10. Its gaps and the v2.1 answers:

1. **Real interviewer.** Defense stage gains an interview mode: a per-stage clock (2:00 for the design, 1:00 per follow-up; overtime is recorded and costs up to 10 points), spoken answers via the browser Web Speech API (feature-detected; transcript lands in the textarea), and **dynamic follow-ups**: with a key, `POST /api/grade` with `stage: "followups"` returns 3 questions aimed at the weakest claim in the learner's own design text (static follow-ups are the fallback). Self-assessment becomes **blind**: the learner scores the rubric first, sees the model answer only afterwards, may revise once, and the record is tagged self-assessed; hints used and reflection retries reduce the defense total (−5 per hint beyond the first, −5 per wrong reflection attempt, floor 0). The account dialog shows whether the grader is on.
2. **Clarification by generation.** Briefs start with a free-text question box. A typed question is matched to the authored clarifications by keyword overlap (reveals the answer and counts as asked); unmatched questions go to `POST /api/clarify` when a key is set (answers in character from the brief's hidden facts, or says the interviewer has no opinion) and otherwise get a neutral reply. The suggestion menu is behind a "Show suggested questions" button that counts as a hint.
3. **Blank-canvas briefs.** `Lesson.blankCanvas: true` makes the starter traffic-only, hides the reference row from Alternatives, and grades only objectives plus runnability. All five briefs use it.
4. **Data systems chapter** (new chapter 10, "Data systems"; toolkit and briefs shift to 11 and 12) with engine support:
   - Queues: `ackMode` (`at-most-once` = today's behaviour, `at-least-once`), `visibilityTimeoutMs`, `maxDeliveries`. A worker replica that dies mid-job (or exceeds the visibility timeout) causes redelivery; redelivered jobs that already completed count as `duplicates` unless the worker is `idempotent` (dedup by key); jobs redelivered more than `maxDeliveries` times are `deadLettered`.
   - Databases: `dbMode: "quorum"` with `quorumWrite` (W) and `quorumRead` (R) over `replicas` (N): a write needs W acks (latency = W-th fastest lane), a read R; fewer than W healthy lanes fails writes; `R + W > N` gives no stale reads, otherwise stale reads follow replication lag. Leader-follower leader death loses writes acked within `replicationLagMs` before the death: `lostWrites`.
   - Gray failures: `slow-server` (like slow-database, for servers), `flapping` (target replica alternates dead/alive every `intervalMs`), `error-burst` (target returns errors for `ratio` of requests while health checks still pass).
   - Connection pools: server `poolSize` bounds in-flight dependency calls per replica; excess calls wait in a pool queue bounded by `maxQueue` (or are rejected as `pool-exhausted` when the bound is hit), so a slow dependency starves an idle server.
   - Tunables: breaker `breakerWindowMs`, `breakerMinCalls`, `breakerFailureRatio`, `breakerOpenMs` per server; workload `deadlineMs`.
   - Lessons: at-least-once-delivery, dead-letter-queue, quorum-reads-and-writes, lost-writes-on-failover, gray-failure, connection-pool-starvation, WRITTEN transactions-and-sagas, WRITTEN stream-processing-and-search.
5. **Usage-based cost.** `cost = provisionedCost + usageCost`; usage is measured operations extrapolated to an hour at per-kind rates per 1,000 operations (database 0.004, writes ×2; cache 0.0005; cdn egress 0.001 per hit; queue 0.0005 per message; server 0.0005; rate limiter 0.0002; cross-region crossing 0.002). Breakdown rows carry `usage`. Existing cost caps are recalibrated so every reference still passes (test-enforced).
6. **Metrics**: `duplicates`, `duplicateRate`, `deadLettered`, `deadLetterRate`, `lostWrites`, `poolRejections`; objectives may target `duplicateRate`, `deadLetterRate`, `lostWrites`.

## 14. v2.2 addendum (2026-09-12): Gemini provider and the interviewer

The v2.1 re-judge stayed at 6.5/10: the canvas cannot express technology selection or data modeling, open-ended design is 9% of the curriculum, the defense is one batched round, and there is no mastery model. The user also requires Google AI Studio (Gemini) for every API call; no Anthropic key anywhere.

1. **Provider.** `@google/genai` replaces `@anthropic-ai/sdk`. `GEMINI_API_KEY` (AI Studio) enables graded mode; `GRADER_MODEL` defaults to `gemini-3.8-flash`. Calls use `ai.models.generateContent({ model, contents, config: { systemInstruction, responseMimeType: "application/json", responseJsonSchema, temperature: 0.3 } })` and parse `response.text`, validated with zod. Every route keeps its 501 self-mode fallback when the key is absent. `GET /api/grade` reports the provider.
2. **Adaptive interviewer.** `POST /api/interview` with `{ lessonId, stage: "turn" | "grade", transcript: [{ role: "interviewer" | "candidate", text }], context: { architecture summary, latest result summary, techChoices, dataModel, estimation outcomes } }`. A turn returns `{ question, intent: "probe" | "pushback" | "escalate" | "clarify", round, done }`: the interviewer reads the canvas and transcript, targets the weakest claim, pushes back at least once, escalates ("traffic is 10×, the cache is cold") by round 3, and stops after 5 rounds. Grade returns rubric item scores, `recoveryScore` 0-2 (did the candidate revise under pushback), `weakConcepts` (rubric ids scored 0), and a critique. Without a key: 501 and the static follow-ups.
3. **Tech and data-model stage** before the defense (mandatory for briefs and blank-canvas runs, optional for sims): for each node kind in the design the learner picks a technology from `lib/tech-catalog.ts` (options with fit traits: read-heavy, write-heavy, hot-keys, strong-consistency, analytics, large-objects, global, streaming) and writes a one-line justification; then a data model (entities, primary keys, query patterns, growth). Both feed the interviewer and grader; self mode reveals the catalog's fit notes after commitment.
4. **Blank-start toggle** on every sim lesson: start from a traffic-only canvas with the same objectives; mission validation is skipped, alternatives omit the reference, progress records `blankCanvas: true`.
5. **One wall clock** per lesson (brief 40 min, sim 15, written 12) shown in the header from the first interaction; overtime saved as `wallClockOvertime`.
6. **Eight more briefs** (Expert, blank canvas, with `techFocus` and `dataModelPrompt`): payments-ledger, ride-matching, collaborative-editing, log-search, video-upload-pipeline, leaderboard, rate-limiter-service, ad-click-aggregation. Readings verified.
7. **Mastery model** (`lib/mastery.ts`): per-concept stats from progress (defense scores, weakConcepts, estimationBias), a spaced review queue (1, 3, 7, 14 days), and a diagnosis line ("you under-estimate database load by 35% on average"). The library shows a Review panel.

## 15. v2.3 addendum (2026-09-13): voice mock, reasoning-graded feedback, a wider world

The v2.2 re-judge scored 7.0/10. Remaining gaps: verbal performance is only proxied; the 8-primitive canvas becomes the learner's vocabulary; consensus is prose-only; no data size or egress in cost; security and migration absent; grading checks metrics, not reasoning; no variance view.

1. **Voice mock** (`/mock`, and "Mock interview" on every brief). One continuous session with phases and a clock: clarify (5 min), estimate (5), design (20), deep dive (10), wrap-up (5); 45 minutes default, 30 available. The candidate speaks (continuous Web Speech recognition; typing allowed) while building on the canvas; the interviewer (Gemini, `POST /api/interview` with `mode: "mock"`, `phase`, `elapsedSeconds`, a canvas snapshot and the transcript) opens, listens, and **interrupts**: after 90 s of candidate speech without a number or a decision, when a claim contradicts the canvas or the last run, or when the phase clock runs out. Transcript stats are computed client-side (words per minute, filler ratio, seconds per phase, numbers stated, signposts such as "first / then / tradeoff"). Final grade = content rubric (existing) + a **performance rubric** graded from the transcript: clarified before designing, stated numbers, signposted, held or revised a position under pressure with reasons, managed time. Without a key: a scripted interviewer with fixed phase prompts and the blind self-assessment; the performance stats still show.
2. **Reasoning graded against the run.** The tech stage collects a one-line rationale and an expected number per component ("cache: expect ~85% hits", "db: ~120 writes/s"). The grader receives the measured per-node metrics and returns `contradictions: [{ nodeId, claim, measured, note }]`; each contradiction costs 5 points. The data model is rubric-scored (`dataModelItems`: keys are unique and stable, partition key matches the hot path, query patterns match the read/write ratio, growth is bounded), replacing the 20-word minimum. Self mode shows the measured numbers next to the claims so the learner sees contradictions themselves.
3. **Wider world.** New kinds: `object-store` (capacity in requests, latency high, requests carry `sizeKb` from workload `payloadKb` and `objectShare`; usage cost = storage GB-month from `storedGb` plus egress per GB served) and `stream` (a partitioned log: `partitions`, `consumerGroups`, per-partition ordering, `retentionSeconds`, replay on consumer restart; a slow partition backs up only its own consumers). Leader election: leader-follower gains `election: "heartbeat" | "consensus"` and `electionMs`; a new failure kind `partition` splits the replicas for `duration`; heartbeat election during a partition can elect two leaders and produce `conflictingWrites`; consensus election waits for a majority and never splits. Cost: storage and egress rows in the breakdown.
4. **Content.** Sim lessons: object-storage-and-egress (video/images: egress dominates), partitioned-streams (ordering, consumer groups, a slow partition), split-brain (heartbeat vs consensus election under a partition). Written: security-and-tenancy (authN/authZ, tokens, tenancy isolation, encryption, secrets, rate limiting per tenant), schema-migration-and-versioning (expand/contract, backfills, dual writes, API versioning and deprecation). All with readings.
5. **Variance view.** Check runs five seeds (the three assessment seeds plus two more); the results show min / median / max for p95, throughput and error rate, and the objectives must pass on all three assessment seeds as before.
