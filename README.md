...
- **70 lessons in 12 chapters**, four tiers from Beginner to Expert: Foundations, Performance, Workloads & queues, Caching deep dive, Reliability, Replication & consistency, Partitioning, Traffic control, Multi-region, Data systems (delivery semantics, dead-letter queues, quorums, lost writes, gray failure, connection pools, object storage and egress, partitioned streams, split-brain, transactions and sagas, streams and search), an Interview toolkit of written knowledge lessons (framing, numbers, API design, storage engines, observability, security and tenancy, schema migration and versioning), and thirteen open-ended Design briefs on a blank canvas.
...
- **Mock interview** at `/mock`: one continuous 45-minute (or 30-minute) spoken session with phase clocks (clarify, estimate, design, deep dive, wrap), an interviewer that listens, probes, pushes back and interrupts, a live canvas you build while you talk, and a grade that scores content, recovery under pressure, technology and data-model choices, contradictions between your claims and your measured run, and performance (clarified first, stated numbers, signposted, held a position, managed time) with transcript stats. Without a key the session runs scripted.
- **Remix and blank start.** Every sim lesson can be replayed with a re-rolled workload, or started from an empty canvas with the same targets; targets are derived from a hidden reference solution so the remix is always solvable and never the same numbers twice.
...
- **Engine 2.3.** Heavy-tailed service times; object stores with payload transfer time, storage and egress cost; partitioned streams with consumer groups, per-partition ordering and replay; leader election by heartbeat or consensus with network partitions and split-brain conflicts; at-least-once delivery with visibility timeouts, redelivery, duplicates, idempotent workers and dead-letter queues; quorum databases with tunable W and R; lost writes on leader failover; gray failures (slow servers, flapping replicas, error bursts that health checks miss); connection pools that starve on slow dependencies; tunable circuit breakers and deadlines; keyed caches with hot keys, TTLs, cold starts and request coalescing; leader-follower databases with replication lag, read-your-writes and failover; sharded databases with hash or range partitioning and hot shards; timeouts, retries with backoff and jitter, circuit breakers; bounded queues and token-bucket rate limiters that shed instead of timing out; load balancers with health-check delay and least-connections; parallel fan-out; regions with cross-region latency and geo routing; failure events that recover (server, database, cache flush, slow database, region outage).
- **Cost 2.3.** Per-kind nonlinear provisioned pricing plus usage-based cost from measured traffic (database operations, cache and CDN hits, queue deliveries, cross-region crossings, bytes stored and bytes served), so a cache, a read replica, a bigger database and a queue are genuinely different strategies, with a per-component breakdown of both.
