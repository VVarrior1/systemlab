...
import { Activity, ArrowDownRight, ArrowRight, ArrowUpRight, Ban, Check, CircleAlert, Clock3, GitCompareArrows, History, Info, Merge, Pin, RefreshCw, ScanLine, ShieldCheck, Timer, Unplug, X } from "lucide-react";
...
export function Results({ result, baseline, workload, baselineWorkload, onPin, onUnpin, stale, running, alternatives, alternativesLoading }: { result: SimulationResult | null; baseline: SimulationResult | null; workload: Workload | null; baselineWorkload: Workload | null; onPin: () => void; onUnpin: () => void; stale: boolean; running: boolean; alternatives?: AlternativeView[] | null; alternativesLoading?: boolean }) {
  ...
  const metrics = [
    ...
    { label: "INFRASTRUCTURE", value: result ? result.cost : null, unit: "credits", previous: baseline?.cost, lower: true, breakdown: result?.costBreakdown },
  ];
  ...
  return <section className="results-section" aria-label="Simulation results">
    ...
    {tab === "alternatives" && <div className="alternatives-panel" role="tabpanel">{alternativesLoading ? <div className="panel-empty"><ArrowRight size={16} className="spin" /><span>Running alternative designs on your workload...</span></div> : alternatives && alternatives.length > 0 ? <div className="alt-table-wrap"><table className="alt-table"><thead><tr><th>Design</th><th>Cost</th><th>P95</th><th>Error rate</th><th>Rejected</th><th>Objectives</th></tr></thead><tbody>
      <tr className="alt-row-you"><td><strong>Your design</strong><span className="alt-rationale">The architecture you built.</span></td><td className="mono">{result ? fmt(result.cost, 1) : "--"}</td><td className="mono">{result?.completed ? `${Math.round(result.p95)} ms` : "--"}</td><td className="mono">{result ? `${(result.errorRate * 100).toFixed(1)}%` : "--"}</td><td className="mono">{result ? `${(result.rejectedRate * 100).toFixed(1)}%` : "--"}</td><td className="mono">--</td></tr>
      {alternatives.map((alt) => <tr key={alt.id}><td><strong>{alt.title}</strong><span className="alt-rationale">{alt.rationale}</span>{alt.comparison && <p className="alt-comparison">{alt.comparison}</p>}</td>{alt.error || !alt.result ? <td colSpan={5} className="alt-error"><CircleAlert size={13} />{alt.error ?? "Could not simulate this alternative."}</td> : <><td className="mono">{fmt(alt.result.cost, 1)}</td><td className="mono">{alt.result.completed ? `${Math.round(alt.result.p95)} ms` : "--"}</td><td className="mono">{`${(alt.result.errorRate * 100).toFixed(1)}%`}</td><td className="mono">{`${(alt.result.rejectedRate * 100).toFixed(1)}%`}</td><td className="mono">{alt.passed}/{alt.total}</td></>}</tr>)}
