...
function KindBadge({ kind }: { kind: LessonKind }) {
  ...
  return <span className={`kind-badge kind-${kind}`}><Icon size={11} />{kindLabels[kind]}</span>;
}
...
export function Library({ view }: { view: "learn" | "designs" }) {
  ...
  const [gymSolved, setGymSolved] = useState(0);
  const [query, setQuery] = useState("");
  const [filter, setFilter] = useState("all");
  const [tier, setTier] = useState<"all" | Lesson["difficulty"]>("all");
  const [kind, setKind] = useState<"all" | LessonKind>("all");
  const [error, setError] = useState("");
  const [deleting, setDeleting] = useState<SavedDesign | null>(null);
  const upload = useRef<HTMLInputElement>(null);
  useEffect(() => {
    const refresh = () => { setProgress(readProgress()); setDesigns(readDesigns()); setGymSolved(Object.values(readGym().solved).reduce((sum, count) => sum + count, 0)); const warning = getPersistenceWarning(); if (warning) setError(warning); };
